...
function action(env: Record<string, string>) {
  return spawnSync("bash", [path.join(root, "scripts", "action.sh")], {
    cwd: proj,
    env: { ...process.env, PERMIT_BIN: path.join(root, "dist", "cli", "main.js"), ...env },
    encoding: "utf8",
  });
}
...
describe("GitHub Action", () => {
  it("action.yml passes every input through env, never into the script body", () => {
    ...
    expect(yml).toContain("PERMIT_PATHS: ${{ inputs.paths }}");
    expect(yml).toContain("PERMIT_ARGS: ${{ inputs.args }}");
    expect(yml).toContain("PERMIT_FAIL_ON_FINDINGS: ${{ inputs.fail-on-findings }}");
  ...
  it("runs permit with the github format and fails on findings", () => {
    execFileSync("npm", ["run", "build"], { cwd: root, stdio: "ignore" });
    const r = action({ PERMIT_PATHS: "src", PERMIT_ARGS: "--today 2026-08-23" });
  ...
  it("fail-on-findings=false reports but exits 0", () => {
    const r = action({ PERMIT_PATHS: "src", PERMIT_ARGS: "--today 2026-08-23", PERMIT_FAIL_ON_FINDINGS: "false" });
    expect(r.status).toBe(0);
    expect(r.stdout).toContain("2 denied");
  });

  it("a malicious-looking input is an argument, not a command", () => {
    const r = action({ PERMIT_PATHS: "src; echo INJECTED", PERMIT_ARGS: "--today 2026-08-23" });
...
describe("pre-commit hook", () => {
  it("declares the permit entry for JavaScript files", () => {
    const y = fs.readFileSync(path.join(root, ".pre-commit-hooks.yaml"), "utf8");
    expect(y).toContain("id: permit");
    expect(y).toContain("entry: permit");
